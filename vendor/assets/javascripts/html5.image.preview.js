...
function previewImage(el,widths,limit){
  ...
  reader.onload = (function(aImg) {
    return function(e) {
      ...
      var description = document.createElement('p');
      description.innerHTML = '<br />图片格式为: <b>'+format+'</b> image, 文件大小 <b>'+(e.total/1024).toFixed(2)+'</b> KB.';
      ...
      if (allowedTypes.indexOf(format)>=0 && e.total<(limit*1024*1024)){
        ...
        description.innerHTML += '<br /><span style="color:green;">图片好像能够正常上传.</span>';
      } else {
          description.innerHTML += '<br /><span style="color:red;">错误的格式或大小! 接受的图片格式为: '+allowedTypes.join(', ')+'. 大小限制为: '+limit+'MB</span>';
      }
